...
def traza_to_plot(path, dump_path,bench):
    ...
    for elem in all_opcodes:
        ...
        if cons_bet_wr[elem][0] == 0 and cons_bet_wr[elem][1] == 0 and cons_bet_wr[elem][2] == 0:
            ...
            totals_r =  cons_bet_wr[elem][0] + cons_bet_wr[elem][1] + cons_bet_wr[elem][2] + cons_bet_wr[elem][3]
        ...
        if cons_bet_wc[elem][0] == 0 and cons_bet_wc[elem][1] == 0 and cons_bet_wc[elem][2] == 0:
            ...
            totals_c = cons_bet_wc[elem][0] + cons_bet_wc[elem][1] + cons_bet_wc[elem][2] + cons_bet_wc[elem][3]
    ...
    for key in dict_to_it:
        if key != "x86_uinst_effaddr":
            if key in sec_to_it:
                ...
                percentil[key] = np.append(dict_to_it[key], sec_to_it[key])
                #print("IF")
            else:
                percentil[key] = np.append(dict_to_it[key], [0,0,0,0])
    ...
    for key in percentil:
        ...
        #print(total)
    for key in percentil:
        percentil[key][0] = percentil[key][0] + percentil[key][4]
        percentil[key] = np.delete(percentil[key], 4)
    ...
    vals = np.array(vals)
    fig, ax = plt.subplots()
    plt.bar( range(len(opc)), vals[:,0],label="0 reads")
    plt.bar(range(len(opc)), vals[:, 1], bottom=vals[:, 0],label="1 read(c)")
    plt.bar(range(len(opc)), vals[:, 2], bottom=vals[:, 0]+vals[:, 1],label="2 reads(c)")
    plt.bar(range(len(opc)), vals[:, 3], bottom=vals[:, 0] + vals[:, 1]+vals[:, 2],label="3 or + reads(c)")
    plt.bar(range(len(opc)), vals[:, 4], bottom=vals[:, 0] + vals[:, 1] + vals[:, 2]+ vals[:, 3],label="1 read(r)")
    plt.bar(range(len(opc)), vals[:, 5], bottom=vals[:, 0] + vals[:, 1] + vals[:, 2] + vals[:, 3] + vals[:, 4],label="2 reads(r)")
